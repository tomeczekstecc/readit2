...
const topSubs = async (_: Request, res: Response) => {
  try {
    ...
    const subs = await getConnection()
      .createQueryBuilder()
      .select(
        `s.title, s.name,if(s.imageUrn != '', concat('${process.env.APP_URL}/images/',s.imageUrn), 'https://www.gravatar.com/avatar/00000000000000000000000000000000?d=mp&f=y') as imageUrl, count(p.id) as postCount`
      )
      .from(Sub, 's')
    ...
    return res.status(500).json({ error: 'Something went wrong', orE:err.message });
  }
};
